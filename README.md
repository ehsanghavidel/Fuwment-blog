...
**فهرست CTA** در `BRAND_CTAS` داخل [company.ts](src/lib/company.ts) است — جدا از پرامپت،
چون یک قاعده‌ی کسب‌وکار است نه بخشی از هنر نویسندگی. [brand-cta.ts](src/lib/agents/brand-cta.ts)
لایه‌ی رندر آن برای پرامپت‌هاست و هر چهار کانال (بلاگ، اینستاگرام، لینکدین، ریلز) از
همان یک منبع می‌خوانند. یک چک قطعی هم می‌سنجد که مدل CTAیی بیرون از فهرست نسازد.
اگر مسیر محتوا مشخص باشد، فهرست با `ctasForRoute()` به همان مسیر فیلتر می‌شود.
...
2. `STUDIO_PASSWORD` را حتماً ست کنید — در پروداکشن نبودش **دسترسی را می‌بندد** (fail-closed)، نه باز.
3. کرون هفتگی فعلاً غیرفعال است؛ `vercel.json` خالی است. برای برگرداندنش، توضیح بالای [`src/app/api/cron/weekly/route.ts`](src/app/api/cron/weekly/route.ts) را ببینید.

### ⚠️ کجا اجرا می‌شود، کجا فقط نمایش داده می‌شود

این تفکیک عمدی است و اگر رعایت نشود، اجرا روی Vercel با تایم‌اوت می‌شکند:
...
روی لپ‌تاپ هیچ سقفی نیست. چون استودیو وضعیت را از Supabase می‌خواند (ارکستریتور هر گام را همان‌جا آینه می‌کند)، اجرای محلی از همان استودیوی مستقر روی Vercel هم قابل تماشاست.

اگر روزی به Pro رفتید، `maxDuration` تا ۸۰۰ ثانیه بالا می‌رود و اجرا روی Vercel هم ممکن می‌شود.

### مقاوم‌سازی‌هایی که سر همین مسئله اضافه شد

این‌ها فارغ از محل اجرا مفیدند و نباید برداشته شوند:

- **سقف زمان روی هر فراخوانی مدل** ([`src/lib/ai.ts`](src/lib/ai.ts)) — ۴۵ ثانیه، و ۹۰ ثانیه برای نویسنده. در یک اندازه‌گیری واقعی، یک تماس **۱۲۷ ثانیه** هنگ کرد و پاسخ خالی داد؛ تلاش بعدی در ۸٫۶ ثانیه جواب داد. بدون سقف، یک هنگ تصادفی به‌تنهایی کل بودجه را می‌خورد. قطع‌شدن‌ها با پیشوند `[llm-timeout]` لاگ می‌شوند.
- **محافظ مهلت** ([`src/lib/agents/run-steps.ts`](src/lib/agents/run-steps.ts)) — پیش از هر گام، اگر وقت کافی نمانده باشد اجرا تمیز با پیام روشن می‌ایستد به‌جای کشته‌شدن ناگهانی و ماندنِ ابدی روی «running».
- **`POST /api/pipeline/run` بلافاصله ۲۰۲ برمی‌گرداند** و استودیو وضعیت را poll می‌کند — اتصال HTTP چنددقیقه‌ای باز نمی‌ماند.
- **تفکیک شدت چک‌های برند** ([`src/lib/agents/brand-checks.ts`](src/lib/agents/brand-checks.ts)) — فقط ایرادهای `blocking` بازنویسی می‌سازند. در یک اجرا، واژه‌ی «مشتریان» دو دور کامل (۶۶ ثانیه از ۲۰۱) هزینه ساخت در حالی که ویراستار از پاس اول تأیید کرده بود.
- **اصلاح خودکار واژگانی** — چند جایگزینی کاملاً بی‌ابهام در کد انجام می‌شود، نه با یک دور بازنویسی گران.
...
│   │   ├── brand-cta.ts          ← رندر دعوت‌به‌اقدام‌ها برای پرامپت (هر چهار کانال)
