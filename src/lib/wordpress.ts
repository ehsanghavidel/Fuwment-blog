 ...
 *
 * ⚠️ پوشش `dir="rtl"` را حذف نکن — حتی حالا که ضروری نیست.
 * مقصد قبلی (`fuwment.com`) قالبی با `lang="en-US"` داشت و هیچ `dir` روی
 * صفحه نمی‌گذاشت؛ بدون این پوشش کل مقاله‌ی فارسی چپ‌چین رندر می‌شد.
 * مقصد فعلی (`fa.fuwment.com`) خودش `dir="rtl" lang="fa-IR"` است (با
 * گرفتن HTML صفحه‌ی اصلی تأیید شد، ۲۸ اوت ۲۰۲۶) — یعنی پوشش دیگر لازم
 * نیست، ولی چون روی خودِ محتوا می‌نشیند مستقل از قالب کار می‌کند و با
 * عوض‌شدن قالب یا مقصد نمی‌شکند. هزینه‌اش صفر است، پس می‌ماند.
 *
