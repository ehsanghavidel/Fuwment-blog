 ...
 * مدل) و چند دقیقه طول می‌کشد. سقف تابع Vercel روی Hobby ۳۰۰ ثانیه است
 * (Pro تا ۸۰۰).
 *
 * ⚠️⚠️ و وقتی تابع به سقف برسد، Vercel آن را **می‌کشد** — کار در پس‌زمینه
 * ادامه پیدا نمی‌کند. (نسخه‌ی قبلی همین کامنت هم عدد را ۶۰ ثانیه گفته بود
 * و هم ادعا می‌کرد اجراها پشت سر درخواستِ قطع‌شده ادامه می‌یابند؛ هر دو
 * غلط بود. همان توهمی است که CLAUDE.md درباره‌ی `waitUntil` هشدار داده:
 * زودبرگرداندن پاسخ، کار را از سقف معاف نمی‌کند.)
 *
 * چیزی که واقعاً از «اجرای تا ابد running» جلوگیری می‌کند، محافظ مهلت در
 * `run-steps.ts` است: پیش از هر گام `getDeadline()` را می‌پرسد و اگر کمتر
 * از ۴۰ ثانیه مانده باشد، همان‌جا تمیز می‌ایستد و اجرا با خطای روشن ثبت
 * می‌شود. استودیو هم به پاسخِ این درخواست تکیه نمی‌کند و وضعیت را از
 * دیتابیس poll می‌کند — که برای اجرای محلی (حالت عادی این پروژه) هم
 * همان‌طور کار می‌کند.
 */
