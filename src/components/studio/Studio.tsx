...
/**
 * استودیوی محتوا — شش تب:
 * ۱. خط تولید: اجرای پایپ‌لاین بلاگ و تماشای زنده‌ی کار ایجنت‌ها
 * ۲. شبکه‌های اجتماعی: چهار حالت تولید محتوای اجتماعی
 * ۳. هفته: برنامه‌ریزی و اجرای هفت کاروسل اینستاگرام
 * ۴. کمپین: یک تم → هر سه کانال با یک روایت مشترک
 * ۵. پست‌ها: مدیریت انتشار (human-in-the-loop) و ثبت بازخورد
 * ۶. درس‌ها: حافظه‌ی خودبهبودی سیستم
 */

type Tab = "run" | "social" | "weekly" | "campaign" | "posts" | "lessons";
...
const TABS: { id: Tab; label: string; icon: (p: { className?: string }) => JSX.Element }[] = [
  ...
  { id: "weekly", label: "هفته", icon: IconRefresh },
  { id: "campaign", label: "کمپین", icon: IconLayers },
...
export function Studio() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      {tab === "weekly" && <WeeklyPanel onUnauthorized={onUnauthorized} />}
      {tab === "campaign" && <CampaignPanel onUnauthorized={onUnauthorized} />}
